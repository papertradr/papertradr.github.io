...
```
Whenever the futures price changes, these securities *should* technically follow the futures price movement. However, it is up to the liquidity providers and market makers to quote them. If they are too slow to follow the futures price movement, scalpers can take advantage of the price difference, which is what we will be implementing. We can summarize the relationship of all the ETPs and the futures contract in a graph:

<figure>
  <img src="/assets/images/Kosdaq150_graph.png" style="width:120%;height: auto"/>
</figure>

The ETPs *depend* on the future's price movement, hence, whenever the futures price changes, we need to check the price of all ETPs. If one of the ETP prices changes, we only need to check that particular ETP. Using the `Graph`, `Node`, and `Relation` classes (available in our `graphlib.py`), we can create a graph of the above securities in python:
```py
